  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        // ✅ Process users to include lastTradeDate AND balance
        const usersWithBalance = await Promise.all(
          (data.users || []).map(async (user) => {
            try {
              // Fetch the user's balance from dashboard
              const balanceRes = await fetch(`http://localhost:5000/api/users/${user.id}/dashboard`, {
                credentials: 'include'
              });
              const balanceData = await balanceRes.json();
              let totalBalance = 0;
              if (balanceData.success && balanceData.dashboard?.balances) {
                totalBalance = balanceData.dashboard.balances.reduce((sum, acc) => sum + (acc.balance?.totalBalance || 0), 0);
              }
          ...
          <table className="w-full text-white table-fixed border-collapse">
            ...
            <tbody>
              {filteredUsers.map((user) => {
                ...
                const totalBalance = user.totalBalance || 0; // ✅ Get the total balance
                return (
                    ...
                    <td className="px-2 py-2">
                      <div className="flex gap-1 justify-between">
                        <button
                          ...
                          disabled={!!actionLoading[`${user.id}_pause`]}
                          className={`px-3 py-2 rounded text-sm flex-1 ${
                            actionLoading[`${user.id}_pause`]
                              ? "opacity-60"
                              : user.status === "paused"
                        ...
                        <button
                          ...
                          disabled={!!actionLoading[`${user.id}_disable`]}
                          className={`px-3 py-2 rounded text-sm flex-1 ${
                            actionLoading[`${user.id}_disable`]
                              ? "opacity-60"
